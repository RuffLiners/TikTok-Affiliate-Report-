...
function buildPrompt(w: ReturnType<typeof buildWindows>): string {
  return `You are a data extraction and analysis agent for the ${BRAND_NAME} TikTok Shop weekly report.
  ...
  try {
    apiRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `Run the full weekly report for ${BRAND_NAME} TikTok Shop. Today is ${format(today, 'MMMM d, yyyy')}. Use the date windows in the system prompt exactly. Complete all 3 phases: run all queries, write all 3 analyses, then output the complete JSON.`
        }],
        mcp_servers: [{
          type: 'url',
