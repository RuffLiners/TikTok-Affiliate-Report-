...
import { REPORT_TZ, todayInReportTz, buildWindows, assemble } from '@/lib/assembleReport'
...
// Window derivation (buildWindows/todayInReportTz) and report assembly
// (assemble) live in src/lib/assembleReport.ts so parity/golden scripts run
...
[{"id":0,"name":"","agent_type":"outreach","campaign_type":"","status":"running","date_posted":"YYYY-MM-DD","gmv_filter":"","kw_filter":"","other_filters":"","list_segment":"","commission_display":"","creators_reached":0,"remaining":0,"total_invites":0,"accepted_invites":0,"total_replies":0,"samples_requested":0,"samples_shipped":0,"total_videos":0,"total_revenue":0,"product_count":0,"has_followups":false}]`
  ...
  19: {
    ...
    prompt: (w, pd) => {
      ...
      // trailing-30-day total off as a weekly number
...
export async function POST(req: NextRequest) {
  ...
  try {
    if (target === 19 || target === 20) {
      ...
      // informational notes — a known one-time correction must not re-fire
      // needsReview on every subsequent run
