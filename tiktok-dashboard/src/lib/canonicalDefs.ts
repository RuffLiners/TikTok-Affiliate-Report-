...
- AFFILIATE GMV (d30.gmv) = bare SUM(gmv) from creator_store_performance in the window. This is authoritative, and it includes video + livestream + showcase creator GMV. The separate d30.affiliateGmv field = the dashboard overview's totalAffiliateGMV, a differently-attributed Euka dashboard metric that runs LOWER than SUM(gmv) by design — the two are DIFFERENT metrics from DIFFERENT sources and are not expected to match. Tier GMV decomposes d30.gmv (SUM(gmv)), never d30.affiliateGmv.
